...
interface AiAssistantProps {
  /** Desk ke andar embed — full-page chrome / outer nav hide. */
  embedded?: boolean;
}
...
export default function AiAssistant({ embedded = false }: AiAssistantProps) {
  ...
  return (
    <div className={`flex ${embedded ? "h-full min-h-0" : "h-dvh"}`} style={{ background: embedded ? "transparent" : "var(--bg-primary)" }}>
      <Sidebar
      ...
      <div className="flex flex-col flex-1 min-w-0 h-full min-h-0">
        ...
        <header
          ...
          style={{ borderBottom: "1px solid var(--bg-hover)", background: embedded ? "rgba(8, 11, 16, 0.6)" : "var(--bg-primary)" }}
        >
          <button
          ...
          {!embedded && (
            <nav className="flex items-center gap-1 mr-1">
              <HeaderNavLink
                ...
                onClick={() => router.push("/trade?tab=portfolio")}
              />
              <HeaderNavLink
                label="Crypto Trade"
